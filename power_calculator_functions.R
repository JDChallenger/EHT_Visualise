...
simulate_trial_IRS <- function(trial_days, n_arms, rep_arms = 4, mos_det = 0, meanMos = 10,
  ...
  for(i in 1:trial_days){
    mosdata[mosdata$night == i,]$net = aux
  }
  
  mosdata$sleeper <- NA
  mosdata$sleeper <- ((mosdata$hut + mosdata$night) %% nhuts) + 1
  mosdata$observation <- factor(formatC(1:nrow(mosdata), flag="0", width=3))
  mosdata$sleeper <- factor(mosdata$sleeper)
  mosdata$net <- factor(mosdata$net)
    ...
    fit_model <-
      glmer(
        cbind(response, n - response) ~
          net + (1 | observation), #(1 | hut) + (1 | sleeper) + 
